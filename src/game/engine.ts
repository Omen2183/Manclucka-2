...
export function pitsOf(player: Player): readonly number[] {
  return YARDS[player];
}
...
export function legalPits(state: GameState): number[] {
  ...
  const own = YARDS[state.turn].filter((i) => state.pits[i]! > 0);
  if (own.length > 0) return own;
  if (state.rules === "until-empty") {
    return YARDS[otherPlayer(state.turn)].filter((i) => state.pits[i]! > 0);
  }
  return [];
}
...
export function tryMove(state: GameState, from: number): MoveResult | null {
  ...
  if (!next.ended) {
    ...
    if (legalPits(next).length === 0) {
      ...
      if (!next.ended) {
        collectRemaining(next.pits, next.turn);
